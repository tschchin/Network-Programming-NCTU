...
void set_PATH_array(char* PATH, char** PATH_array);
bool checkFileInPATH(char** PATH_array,char* file);
...
int main(void) {
    ...
    char root[20] = "bin:.";
    char* PATH_array[MAX_PATH_LENGTH];

    char shell_prompt[3] = "% ";

    setenv("PATH",root,1);
    set_PATH_array(root,PATH_array);
    ...
    while(1) {
        ...
        while( prc<process_count ) {
            ...
            if( strcmp(process[prc][0],"printenv")==0 ) {
                ...
                if( strcmp(process[prc][1],"PATH")==0 ) { // update path array
                    set_PATH_array(process[prc][2], PATH_array);
                }
            } else if( strcmp(process[prc][0],"exit")==0 ) {
                exit(0);
            } else if( checkFileInPATH(PATH_array,process[prc][0]) ){ // deal with PATH process
                if ( write_file_flag==1 && prc==(process_count-1) ) { /* === redirection == */
                    ...
                    if( pid==0 ) {
                        ...
                        execvp(process[prc][0], process[prc]);
                    } else {
                        //wait(NULL);
...
void set_PATH_array(char* PATH, char** PATH_array) {
    int token_count = 0;
    PATH_array[token_count] = strtok(PATH, ":");
    while( PATH_array[token_count]!=NULL ){
        token_count++;
        PATH_array[token_count] = strtok(NULL, " ");
    }
}
