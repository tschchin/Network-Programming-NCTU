...
#include <errno.h>
#define QLEN 30
#define BROADCAT_MSG_LENGTH 1500
#include "user_3.h"
#include "semaphore.h"
...
int check_name_pipe_exist(int sender, int receiver,User* user);
int check_name_exist(char *name,User* user);
int check_id_exist(int id,User* user);
void broadcast_msg(const char* msg,User* user);

//int name_pipe_record[QLEN+1][QLEN+1] = {0}; // by id
//extern struct User *user;


int npshell(int user_index,User *user) {
    setenv("PATH","bin:.",1);
    ...
    while(1) {
        char line[MAX_INPUT_LENGTH] = {0}; // command from standard input
            ...
            if ( tell_yell_flag==0 ) {
                while( token[i]!=NULL ) {
                    if ( token[i][0]=='<' ) {
                        ...
                        if(check_id_exist(name_pipe_sender_id,user)==0) { // user do not exist
                            printf("*** Error: user #%d does not exist yet. ***\n",name_pipe_sender_id);
                            fflush(stdout);
                            ignore=1;
                            //return cc;
                        } else {
                            if(check_name_pipe_exist(name_pipe_sender_id,user_index+1,user)==1) { // name pipe exist
                                name_pipe_in_flag = 1;
                                sprintf(name_pipe_in_file,"user_pipe/%d-%d",name_pipe_sender_id,user_index+1);
                                sem_wait(clisem);
                                user[user_index+1].name_pipe_record[name_pipe_sender_id] = 0;
                                sem_signal(clisem);
                ...
                if( token[i]!=NULL ) {
                    if( strcmp(token[i],"|")==0 ) {
                        ...
                        if(check_id_exist(name_pipe_out_id,user)==0) { // user do not exist
                            printf("*** Error: user #%d does not exist yet. ***\n",name_pipe_out_id);
                            fflush(stdout);
                            ignore=1;
                            //return cc;
                        } else {
                            if (check_name_pipe_exist(user_index+1, name_pipe_out_id,user)==0) {  // do not have name pipe yet
                                sprintf(name_pipe_out_file,"user_pipe/%d-%d",user_index+1,name_pipe_out_id);
                                int f = open(name_pipe_out_file,O_CREAT, 0664);
                                close(f);
                                sem_wait(clisem);
                                user[name_pipe_out_id].name_pipe_record[user_index+1]=1;
                                sem_signal(clisem);
        ...
        if(ignore==0){
            ...
            while( prc<process_count ) {
                ...
                if( strcmp(process[prc][0],"printenv")==0 ) {
                    ...
                    char msg[BROADCAT_MSG_LENGTH];
                    if(check_id_exist(id,user)==1) {
                        char tell_msg[BROADCAT_MSG_LENGTH]={0};
                        int i=2;
                        ...
                        sem_wait(clisem);
                        sprintf(user[id-1].broadcast_msg[user[id-1].msg_count],msg,strlen(msg));
                        user[id-1].msg_count++;
                        user[id-1].receive=1;
                        kill(user[id-1].pid,SIGUSR1);
                        sem_signal(clisem);
                    } else {
                        printf("*** Error: user #%d does not exist yet. ***\n",id);
                    }
                    ...
                    char msg[BROADCAT_MSG_LENGTH];
                    char yell_msg[BROADCAT_MSG_LENGTH]={0};
                    int i=1;
                    while(process[prc][i]!=NULL) {
                    ...
                    if ( write_file_flag==1 && prc==(process_count-1) ) { // === redirection ==
                        ...
                        if( pid==0 ) { // child
                            ...
                            /*int k=0;
                            while(process[prc][k]!=NULL) {
                                printf("%s ",process[prc][k]);
                                fflush(stdout);
                                k++;
                            }*/
...
int check_name_pipe_exist(int sender, int receiver,User* user) {
    //printf("+++++\n");
    if( user[receiver].name_pipe_record[sender]==1 ) {
...
void broadcast_msg(const char* msg,User* user) {
    ...
    for(int i=0; i<QLEN; ++i) {
        if(user[i].valid==1) {
            ...
            //sem_wait(clisem);
            sprintf(user[i].broadcast_msg[user[i].msg_count],msg,strlen(msg));
            user[i].msg_count++;
            user[i].receive = 1;
